...
class Humanoid(BaseTask):
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless):
        ...
        self.save_render_img = cfg['args'].save_render
        self.save_bvh = cfg['args'].save_bvh
        ...
        self.root_move_xyz_cpu = [-2, 0, 1]
        super().__init__(cfg=self.cfg)
        self.root_move_xy = torch.tensor(self.root_move_xyz_cpu[:2], device=self.device, dtype=torch.float)
        ...
        if self.move_xy:
            self._initial_humanoid_root_states[..., :2] =  self.root_move_xy.clone()
        
        self._initial_humanoid_root_states[..., 2] = self.root_move_xyz_cpu[2]
        self._initial_humanoid_root_states[:, 7:13] = 0

        self._initial_humanoid_root_states[:, 3] = 0
        self._initial_humanoid_root_states[:, 4] = 0
        self._initial_humanoid_root_states[:, 5] = 0
        self._initial_humanoid_root_states[:, 6] = 1
        # print(self._initial_humanoid_root_states[:,3:7])
        ...
        self._paddle_to_hand = torch.tensor([0, 0, -0.16], device=self.device, dtype=torch.float).repeat(self.num_envs,1)
    ...
    def init_save(self):
        ...
        if self.save_render_img or self.save_bvh:
            current_datetime = datetime.datetime.now()
            self.rgb_path = current_datetime.strftime("%d-%H-%M")
            # self.rgb_path = "try"
            os.makedirs(f"saved_results/{self.rgb_path}/",exist_ok=True)
            
        if self.save_bvh:

            self.record_body_pos=[]
            self.record_body_rot=[]

            self.record_body_pos_op=[]
            self.record_body_rot_op=[]

            self.record_qpos = []
            self.record_qpos_op = []
            self.record_ball_pos = []
            self.record_targets = []
            self.record_targets_op = []
    ...
    def _setup_character_props(self, key_bodies):
        ...
        elif (asset_file == "mjcf/pingpong_humanoid.xml"):
            self._dof_body_ids = [1, 2, 3, 4, 6, 7, 8, 9, 10, 11, 12, 13, 14]
            self._dof_offsets = [0, 3, 6, 9, 10, 13, 14, 17, 20, 21, 24, 27, 28, 31]
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        #left hand is 11 geom in .xml, need to count
        prp = self.gym.get_asset_rigid_shape_properties(humanoid_asset)
        prp[11].restitution = 0.2
        prp[11].friction = 1.
        prp[11].rolling_friction = 0.1
        self.gym.set_asset_rigid_shape_properties(humanoid_asset,prp)
    ...
    def _build_env(self, env_id, env_ptr, humanoid_asset):
        ...
        start_pose.p = gymapi.Vec3(self.root_move_xyz_cpu[0], self.root_move_xyz_cpu[1], self.root_move_xyz_cpu[2])
        ...
        # self.gym.set_actor_rigid_shape_properties(env_ptr, humanoid_handle, [rsp])
    ...
    def post_physics_step(self):
        ...
        if self.save_render_img:
            ...
            rgb_filename = f"saved_results/{self.rgb_path}/frame{self.frame_count:04d}.png" 
            self.gym.write_camera_image_to_file(self.sim, self.envs[0], self.camera_handles[0], gymapi.IMAGE_COLOR, rgb_filename)
        ...
        if self.frame_count>self.save_frames: 
            ...
            if self.save_render_img:
                ...
                image_folder = f'saved_results/{self.rgb_path}/'
                image_files = [f for f in os.listdir(image_folder) if f.endswith('.png')]
                image_files.sort()

                video_file = f'saved_results/{self.rgb_path}_output_video.mp4'
                fps = 30
    ...
    def _build_camera(self):
        ...
        cam_props.height = 1024

        cam_pos = gymapi.Vec3(self._cam_prev_char_pos[0] + 5.5, 
        ...
        camera_handle = self.gym.create_camera_sensor(self.envs[0], cam_props)
        self.camera_handles.append(camera_handle)
        
        self.gym.set_camera_location(camera_handle, self.envs[0], cam_pos, cam_target)
