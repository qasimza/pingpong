            ...
            if self.rank == 0:
                ...
                if self.game_rewards.current_size > 0:
                    ...
                    for i in range(self.value_size):
                        self.writer.add_scalar('rewards/frame'.format(i), mean_rewards, frame)
                        self.writer.add_scalar('rewards/iter'.format(i), mean_rewards, epoch_num)
                        self.writer.add_scalar('rewards/time'.format(i), mean_rewards, total_time)
                    ...
                    if self.print_stats:
                        ...
                        print(f'fps step: {fps_step:.1f} fps total: {fps_total:.1f} mean reward:{mean_rewards[0]:.1f} mean lengths:{mean_lengths:.1f}')
                    # print(f'mean reward:{mean_rewards} mean lengths:{mean_lengths}')

                if self.save_freq > 0:
                    if (epoch_num % self.save_freq == 0):
                        self.save(model_output_file)

                        if (self._save_intermediate) and (epoch_num % (self.save_freq * 10) == 0):
                            int_model_output_file = model_output_file + '_' + str(epoch_num).zfill(6)
