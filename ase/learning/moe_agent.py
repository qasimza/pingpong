...
class MOEAgent(common_agent.CommonAgent):
    def __init__(self, base_name, config):
        ...
        self.test_ = hrl_config_params['config']['test']
    ...
    def env_step(self, actions, forehand_actions=None, forehand_slice_actions=None, backhand_actions=None, backhand_slice_actions=None, forehand_smash_actions = None):
        ...
        for t in range(self._llc_steps):
            ...
            llc_actions = weights * llc_actions #+ (1-weights) * llc_forehand_actions
    ...
    def play_steps(self):
        ...
        for n in range(self.horizon_length):
            ...
            # root_transl = self.vec_env.env.task.target_root_transl.clone()
            skill_command = self.obs['obs'][:,-self.skill_command_size:].clone()
            ...
            if self.use_action_masks:
                ...
                if self.test_:
                    action = self.get_action(self.obs, is_determenistic=True)
                # action = self.get_action(self.obs, is_determenistic=True)
                # if len(self.forehand_id)>0:
                hrl_forehand_action = self._hrl_forehand_agent.get_action(self.obs['obs'][:,:-self.skill_command_size], is_determenistic=True)
                hrl_forehand_slice_action = self._hrl_forehand_slice_agent.get_action(self.obs['obs'][:,:-self.skill_command_size], is_determenistic=True)
                hrl_backhand_action = self._hrl_backhand_agent.get_action(self.obs['obs'][:,:-self.skill_command_size], is_determenistic=True)
                hrl_backhand_slice_action = self._hrl_backhand_slice_agent.get_action(self.obs['obs'][:,:-self.skill_command_size], is_determenistic=True)
                hrl_forehand_slmash_action = self._hrl_forehand_smash_agent.get_action(self.obs['obs'][:,:-self.skill_command_size], is_determenistic=True)
            ...
            if self.test_:
                self.obs, rewards, self.dones, infos = self.env_step(action, hrl_forehand_action, hrl_forehand_slice_action, hrl_backhand_action, hrl_backhand_slice_action, hrl_forehand_slmash_action)
            else:
                self.obs, rewards, self.dones, infos = self.env_step(res_dict['actions'], hrl_forehand_action, hrl_forehand_slice_action, hrl_backhand_action, hrl_backhand_slice_action, hrl_forehand_slmash_action)
            # self.obs, rewards, self.dones, infos = self.env_step(res_dict['actions'])
            # self.obs, rewards, self.dones, infos = self.env_step(action,hrl_forehand_action,hrl_backhand_action)
            ...
            # update_indices = done_indices[self.current_lengths[done_indices]>4]
            # if len(done_indices)>0:
            #     print(torch.sum(self.current_lengths[done_indices]<4))
    ...
    def _build_hrl_forehand(self, config_params, checkpoint_file):
        ...
        self._hrl_forehand_agent = hrl_agent.HRLAgent('hrl_forehand', hrl_forehand_agent_config, moe=True)
        self._hrl_forehand_agent.restore(checkpoint_file)

        self._hrl_forehand_agent.set_eval()
        return
    ...
    def _build_hrl_forehand_slice(self, config_params, checkpoint_file):
        ...
        self._hrl_forehand_slice_agent = hrl_agent.HRLAgent('hrl_forehand_slice', hrl_forehand_agent_config, moe=True)
        self._hrl_forehand_slice_agent.restore(checkpoint_file)

        self._hrl_forehand_slice_agent.set_eval()
        return
    ...
    def _build_hrl_forehand_smash(self, config_params, checkpoint_file):
        ...
        self._hrl_forehand_smash_agent = hrl_agent.HRLAgent('hrl_forehand_smash', hrl_forehand_agent_config, moe=True)
        self._hrl_forehand_smash_agent.restore(checkpoint_file)

        self._hrl_forehand_smash_agent.set_eval()
        return
    ...
    def _build_hrl_backhand(self, config_params, checkpoint_file):
        ...
        self._hrl_backhand_agent = hrl_agent.HRLAgent('hrl_backhand', hrl_forehand_agent_config, moe=True)
        self._hrl_backhand_agent.restore(checkpoint_file)

        self._hrl_backhand_agent.set_eval()
        return
    ...
    def _build_hrl_backhand_slice(self, config_params, checkpoint_file):
        ...
        self._hrl_backhand_slice_agent = hrl_agent.HRLAgent('hrl_backhand_slice', hrl_forehand_agent_config, moe=True)
        self._hrl_backhand_slice_agent.restore(checkpoint_file)

        self._hrl_backhand_slice_agent.set_eval()
        return
    ...
    def _build_llc_forehand_slice(self, config_params, checkpoint_file):
        ...
        self._llc_forehand_slice_agent = ase_agent.ASEAgent('llc_forehand_slice', llc_agent_config)
    ...
    def _build_llc_forehand_smash(self, config_params, checkpoint_file):
        ...
        self._llc_forehand_smash_agent = ase_agent.ASEAgent('llc_forehand_smash', llc_agent_config)
        self._llc_forehand_smash_agent.restore(checkpoint_file)
        print("Loaded Forehand LLC checkpoint from {:s}".format(checkpoint_file))
        self._llc_forehand_smash_agent.set_eval()
        return
    ...
    def _build_llc_backhand(self, config_params, checkpoint_file):
        ...
        self._llc_backhand_agent = ase_agent.ASEAgent('llc_backhand', llc_agent_config)
    ...
    def _build_llc_backhand_slice(self, config_params, checkpoint_file):
        ...
        self._llc_backhand_slice_agent = ase_agent.ASEAgent('llc_backhand_slice', llc_agent_config)
    ...
    def _build_hrl_agent_config(self, config_params, network):
        ...
        obs_size = obs_space.shape[0]-self.skill_command_size#-3
